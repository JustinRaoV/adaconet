...
\begin{tablenotes}
\small
AdaCoNet achieves $|\alpha|/p \geq 0.059$ on all v4 configs; Spearman is included in most configs ($f_0 < 0.5$) but excluded on seed~0 of $N{=}500, P{=}500$ and $N{=}1000, P{=}500$ where high zero inflation triggers the $f_0$ guard, and at $P{=}1000$ where $|\alpha|/p < 0.05$. SparCC, FastSpar, and CCLasso were excluded at $P{=}1000$ due to computational cost. Subscripts denote standard deviations across seeds.
\end{tablenotes}
\end{table}
...
AdaCoNet completes in under 3 seconds at $P{=}1000$, while SparCC requires 63--86\,s at $P{=}500$ due to repeated lsq\_linear solves. REBACCA achieves near-instantaneous computation ($<$0.01\,s) through its closed-form Sherman--Morrison solution (Fig.~\ref{fig:pareto}). At $P{=}1000$, SparCC, FastSpar, and CCLasso were excluded from benchmarking due to prohibitive computational cost.
...
A sensitivity analysis sweeping $c_{\text{ref}}$ from 0.01 to 0.15 confirms that the default value $c_{\text{ref}} = 0.05$ sits at a natural transition point. On v4 data ($N{=}500, P{=}500$), seeds with low zero fraction ($f_0 < 0.5$) retain Spearman for $c_{\text{ref}} \leq 0.05$ (AUROC $\approx 0.80$) and lose it at $c_{\text{ref}} \geq 0.07$ (AUROC drops to $\approx 0.74$), a gap of $\sim$0.06 AUROC directly attributable to the inclusion of an informative Spearman layer. On SparseDOSSA2 ($|\alpha|/p = 0.066$, $f_0 = 0.66$), the zero-fraction guard ($f_0 > 0.5$) blocks Spearman inclusion for all tested $c_{\text{ref}}$ values, maintaining AUROC at 0.72 regardless---demonstrating that the two diagnostics are complementary: the $|\alpha|/p$ criterion captures model-fit quality while $f_0$ guards against CLR degradation under extreme sparsity.
